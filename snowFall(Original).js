(() => {
    ...
    function run() {
        ...
        const snowBallList = createSnowBalls(canvas, numberOfSnowBalls);
        snowBallList.forEach(snowBall => {
            drawSnowBall(canvasContext, snowBall);
        });
        setInterval(() => {
            canvasContext.clearRect(0, 0 ,canvas.width, canvas.height);
            snowBallList.forEach(snowBall => { drawSnowBall(canvasContext, snowBall); });
            snowBallList.forEach(snowBall => { moveSnowBall(snowBall, canvas); });
